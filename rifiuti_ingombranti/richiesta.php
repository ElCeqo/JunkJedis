...
if ($dbconn) {
    $q1="select municipio, oggetto, via from prenotazioni where nome=$1 and cognome=$2";
    $result=pg_query_params($dbconn, $q1, array($name,$surname)); 
    if ($result) {
        $tabella="";
        while($row=pg_fetch_array($result, null, PGSQL_ASSOC)) {
            $tabella.="<tr><td>".$row["municipio"]."</td><td>".$row["oggetto"]."</td><td>".$row["via"]."</td></tr>";
        }
    }
}
?>
...
<body >
    ...
    <div class="titoli_centrali"><b><u>Raccolta rifiuti ingombranti</u></b></div>
